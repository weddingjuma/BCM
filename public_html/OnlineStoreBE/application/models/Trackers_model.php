...
class Trackers_model extends CI_Model {
    ...
    public function getAllDataColumns($where, $table, $columns) {
    ...
    public function getAllDataFetchId($where, $table) {
   ...
   ,customerID,Fitting_Center   ,installername  ,confirmedBY,confirmdate  ,DtDobieBooking  ,customerAddress      ,customerEmailAddress");
    ...
    public function getCrmMessage($where, $table) {

        $this->db->limit(1200);
        $this->db->where('userId', $where['userId']);
        $this->db->or_where('receiver', $where['userId']);
    ...
    public function getCrmMessageCount($where, $table) {
    ...
    public function getAllVehicles($where, $table) {
        $this->db->select('TBL_VEHICLES.id ,TBL_VEHICLES.validityEndOn,validityGvEndOn,idNo ,custId,regNo,certNo,make,model,engNo,chassisNo,'
                . 'financierId,insurorId,governorId,TBL_CUSTOMERS.name,nextOfKin,mobile_1');
        $this->db->from($table);
    ...
    public function getAllcustomerJoinVehicles($where) {
        $this->db->select("tbl_vehicles.id ,tbl_vehicles.custId,tbl_vehicles.code  ,regNo,make ,model ,engNo,chassisNo,color,deviceUnique ,dataline      ,approve_date      ,financierId      ,insurorId
      ...
      ,mobile_1      ,mobile_2,idNo      ,nextOfKin      ,nextOfKinMobile_1");
        ...
        $this->db->join('TBL_CUSTOMERS', 'TBL_VEHICLES.custId = TBL_CUSTOMERS.id');
        $this->db->limit(10);
        $this->db->where("TBL_VEHICLES.status < 2 and " . " tbl_vehicles.custId is not null and TBL_VEHICLES.id = " . $where . "");
        $this->db->order_by('TBL_VEHICLES.id', 'desc');
        $q = $this->db->get();
    ...
    public function getAllDataCertificate($where, $tablename, $search) {
        $this->db->select('tbl_certificates.id,vehicleId,tbl_certificates.certNo,tbl_certificates.deviceType,tbl_certificates.dateCreated,approved,duration,PrintingStatus,generated,'
                . 'paidFor,tbl_certificates.status,tbl_certificates.validityBeginOn,tbl_certificates.validityEndOn,tbl_certificates.createdBy,approvedBy');
        $this->db->limit(100);
        $this->db->from($tablename);
        $this->db->join('TBL_VEHICLES', 'TBL_VEHICLES.id = tbl_certificates.vehicleId');
        $this->db->where($where);
        $this->db->like('tbl_certificates.certNo', $search);
        $this->db->or_like('regNo', $search);
        $this->db->order_by('regNo', 'desc');
        $q = $this->db->get();

        return $q->result_array();
    }

    public function searchItem($search, $column) {
    ...
    public function searchService($search, $column) {
        $this->db->select($column);
        $this->db->from("TBL_SERVICES");
        $this->db->like('name', $search);
        $this->db->or_like('desc', $search);
        $this->db->or_like('specs', $search);
    ...
    public function getTableCount($where, $table) {
    ...
    public function insertCustomer_data($in, $tbl) {
    ...
    public function getCRMLocation($imei) {
        $q = $this->db->query("EXEC getLocationStatus '$imei'");
        //$q = $this->db->get();
        return $q->result_array();
    }
